...
<p align="center">
  一个通过追问、解释、回顾与共同修改，陪你构建个人知识库的桌面 AI 伙伴。
</p>
...
> Liora 帮你把“刚刚学到但还没有真正说清楚的内容”，整理成可以长期阅读、搜索、关联和回顾的 Markdown 知识。

它不是单纯保存聊天记录。你先向桌面上的 Liora 复述自己的理解，它会追问、解释或纠正；你确认后，内容才进入知识库。连接 Obsidian 后，Markdown 是长期主数据，本地 Knowledge Engine 负责索引、关系发现、审核和回顾调度。
...
| 你的目的 | 推荐路线 | 需要编程环境吗 |
| --- | --- | --- |
| 只想体验桌宠、反思和知识管理 | 下载 Windows 安装包 | 不需要 |
...
| 只想看 Vault，不运行桌宠 | 单独安装 Obsidian 插件 | 可以浏览本地 Markdown，但智能维护功能不可用 |
...
| --- | --- | --- |
| Liora 桌面端 | Electron 桌宠、对话界面、托盘菜单、语音和天气入口 | 是 |
| Knowledge Engine | 本机 Python 后端，负责知识稿、索引、关联、审核、回顾和 SQLite 状态 | 是，安装版自带独立运行时 |
| Liora Knowledge 插件 | Obsidian 中的 Home、日历备忘、知识管理台和审核界面 | 否，需要单独安装到 Vault |

因此有两个常见但容易混淆的事实：

1. 修改 `obsidian-plugin/` 不会自动改变已经安装的桌宠，也不会让桌面安装包变大；插件需要重新构建并复制到 Vault。
